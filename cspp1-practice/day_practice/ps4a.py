...
def isValidWord(word, hand, wordList):
    ...
    hand_copy = getFrequencyDict(word)
    for each_char in hand_copy:
        if each_char not in hand:
            return False
        if hand_copy.get(each_char) > hand.get(each_char):
            return False
    return True
...
def playGame(wordList):
    ...
    user_hand = None
    while True:
        user_option = input("Enter n (new Hand) | r (last hand again) | e (exit)")
        if user_option == 'n':
            user_hand = dealHand(HAND_SIZE)
            playHand(user_hand, wordList, HAND_SIZE)
            # user_hand = playHand(user_hand, wordList, HAND_SIZE)
        elif user_option == 'r':
            playHand(user_hand, wordList, HAND_SIZE)
            # user_hand = playHand(user_hand, wordList, HAND_SIZE)
        elif user_option == 'e':
